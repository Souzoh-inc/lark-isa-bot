...
// 構造: <li><div class="date">日付</div><div class="cat"><span class="catXX">カテゴリ</span></div><div class="txt"><a href="URL">テキスト</a></div></li>
function parseNewsItems(rawHtml) {
  ...
  while ((liMatch = liRegex.exec(html)) !== null) {
    const li = liMatch[0];
    const dateMatch = li.match(/<div class="date">([^<]*)<\/div>/);
    const catMatch = li.match(/<span class="cat\d+">([^<]*)<\/span>/);
    ...
    if (dateMatch && linkMatch) {
      ...
      if (date.match(/\d{4}年/)) items.push({ date, category, text, href });
    }
  }
  return items;
}
...
function filterRecentNews(items, daysBack = 2) {
  ...
  return items.filter(item => {
    ...
    if (!m) return false;
    return new Date(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3])) >= cutoff;
  });
}

// --- 影響判定 ---
function analyzeImpact(text) {
  const high = ["登録支援機関", "特定技能", "定期届出", "届出", "支援計画", "受入れ機関"];
...
function buildLarkCard(importantNotices, recentNews, allNews) {
  ...
  const displayNews = recentNews.length > 0 ? recentNews : allNews.slice(0, 5);
  if (recentNews.length === 0 && allNews.length > 0) {
    newsMd += "_直近2日間の新規掲載はありませんでした。最新の更新情報:_\n\n";
  }
  for (const item of displayNews.slice(0, 6)) {
    ...
    if (impact === "high") {
      ...
      newsMd += "→ " + item.category + "。登録支援機関への直接的な制度変更はなし。\n";
    }
    newsMd += "\n";
  }
  ...
  // === 登録支援機関への影響 ===
  const allItems = [...displayNews, ...importantNotices];
  ...
  impactMd += "🔴 **やるべきこと（法的義務）：** " + (highItems.length > 0 ? "\n" + highItems.map(i => "・" + i.text).join("\n") : "なし") + "\n\n";
  impactMd += "🟡 **やらないといけないこと（届出・手続き）：** " + (medItems.length > 0 ? "\n" + medItems.map(i => "・" + i.text).join("\n") : "なし") + "\n\n";
  ...
  if (isMonday) {
    reminderMd += "\n📌 **今週の注意:** 定期届出の進捗確認・クライアント企業への周知状況を確認しましょう。\n";
  }
  elements.push({ tag: "markdown", content: reminderMd });
  elements.push({ tag: "hr" });

  // === クイックリンク ===
  elements.push({ tag: "markdown", content: "🔗 [入管庁トップ](https://www.moj.go.jp/isa/index.html)｜[特定技能制度](https://www.moj.go.jp/isa/applications/ssw/index.html)｜[省令改正](https://www.moj.go.jp/isa/10_00222.html)" });
  elements.push({ tag: "note", elements: [{ tag: "plain_text", content: "出典: 出入国在留管理庁公式サイト (www.moj.go.jp/isa/) | 自動配信レポート" }] });

  return {
    msg_type: "interactive",
    card: {
      header: { title: { tag: "plain_text", content: "入管庁ニュース（" + dateStr + "）" }, template: "blue" },
      elements,
    },
  };
}
...
export default async function handler(req, res) {
  try {
    console.log("Daily report v4: starting...");
    ...
    console.log("Lark result: " + JSON.stringify(result));
